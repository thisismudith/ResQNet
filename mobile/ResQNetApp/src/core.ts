...
export default class CORE {
  ...
  initialize() {
    ...
    this.subscriptions.push(
      this.eventEmitter.addListener('onMessageReceived', payload => {
        let data = JSON.parse(payload.message);
        data.deviceId = payload.endpointId; // Attach sender ID
  ...
  start() {
    ...
    console.log('Started advertising to nearby devices');
  }
  ...
  loop() {
    ...
    clearTimeout(this.loop1Var);
    this.loop1Var = setTimeout(() => this.loop(), this.wait_interval() || 5e3);
  }
  ...
  loop2() {
    ...
    clearTimeout(this.loop2Var);
    this.loop2Var = setTimeout(() => this.loop2(), 1000); // This loop runs more frequently
  }
  ...
  broadcast() {
    ...
    this.devices.forEach(d => {
      ...
      for (
        let i = this.broadcastedMsgIndex;
        i < this.receivedMessage.length;
        i++
      ) {
        const msg = this.receivedMessage[i];
        console.log('Sending message to', d.endpointId, msg);
        NearbyConnection.sendMessage(d.endpointId, JSON.stringify(msg));
        this.broadcastedMsgIndex++;
      }
    });
    this.broadcastedMsgIndex = this.receivedMessage.length;
  }
