...
window.CoreAPI = CoreAPI; // for debug
...
export default function SOS() {
  ...
  const [msgD, setmsgD] = useState({ current: 0, total: 0 });
  ...
  useEffect(() => {
    setmsgD({
      current: CoreAPI.broadcastedMsgIndex,
      total: CoreAPI.receivedMessage.length,
    });
  }, [CoreAPI.broadcastedMsgIndex, CoreAPI.receivedMessage.length]);
    ...
    <SafeAreaView edges={['top']} style={styles.safe}>
      ...
      >
        ...
        <Text style={styles.sub}>
          {' '}
          Tap SOS to arm. It will send in 3 seconds. Tap again to cancel before
          it sends.
        </Text>
        ...
        <Text>{CoreAPI.isActive ? 'Broadcasting' : 'Not broadcasting'}</Text>
        <Text>
          {msgD.current} / {msgD.total}
        </Text>
